...
setwd("/mnt/Win10/Users/Pavel/Omega/USM PhD/Projects/Active/Blackfin Tuna/Analyses/extramiss/DAPC/differentpops")
setwd(getwd())
##### Load in data and create backup object


infile <- "../../data/kinless.neutral.gen"
...
ggsave("DAPC_missingrm.pc98.png", height = 7, width = 11.5, units = "in")
...
fig <- plot_ly(locdf, x = ~LD1, y = ~LD2, z = ~LD3, color = ~population, colors = my_pal[c(1,6,3,9)], size = 15)
fig <- fig %>% add_markers()
...
fig
htmlwidgets::saveWidget(as_widget(fig), paste0("dapc_", "neutral", ".html"))
