...
setwd(getwd())
k2 <- read.table("bft_maf01_kinless_outlier.faststructure_out.2.meanQ")
k3 <- read.table("bft_maf01_kinless_outlier.faststructure_out.3.meanQ")
k4 <- read.table("bft_maf01_kinless_outlier.faststructure_out.4.meanQ")
k5 <- read.table("bft_maf01_kinless_outlier.faststructure_out.5.meanQ")
k6 <- read.table("bft_maf01_kinless_outlier.faststructure_out.6.meanQ")
k7 <- read.table("bft_maf01_kinless_outlier.faststructure_out.7.meanQ")
k8 <- read.table("bft_maf01_kinless_outlier.faststructure_out.8.meanQ")
k9 <- read.table("bft_maf01_kinless_outlier.faststructure_out.9.meanQ")
...
colnames(k2) <- c("sample", "population", "probability")
...
#### individual plots ####
k2$population <- as.factor(k2$population)
levels(k2$population) <- c("1","2")
...
ggplot(k2, aes_string(fill = "population", y = "probability", x = "sample")) +
    ...
    coord_cartesian(ylim = c(0, 1), expand = FALSE, clip = "off") +
    annotate("text", x = poptext[1], y = 0.1, label = "BRZ", color = "#D8DEE9" ) +
    annotate("text", x = poptext[2], y = 0.1, label = "KEY", color = "#D8DEE9") +
    annotate("text", x = poptext[3], y = 0.1, label = "MRT", color = "#D8DEE9") +
    annotate("text", x = poptext[4], y = 0.1, label = "PNS", color = "#D8DEE9") +
    annotate("text", x = poptext[5], y = 0.1, label = "PR", color = "#D8DEE9") +
    annotate("text", x = poptext[6], y = 0.1, label = "SCA", color = "#D8DEE9") +
    annotate("text", x = poptext[7], y = 0.1, label = "TX", color = "#D8DEE9") +
    annotate("text", x = poptext[8], y = 0.1, label = "VZ", color = "#D8DEE9") +
    theme(
